...
namespace Lib_Creator
{
    public class DBCWriter
    {
        ...
        public static DataTable GetData(string filename)
        {
            ...
            {
                byte[] rowData = br.ReadBytes(rowlen);//读取一行的数据
        ...
        private static int[] findTextColumns(int startIndex, DataTable dt, Dictionary<int, string> dic_textData)
        {
            ...
            for (int rowindex = 0; rowindex < dt.Rows.Count; rowindex++)
            {
                ...
                {
                    int str_index = int.Parse(dr[colindex].ToString());//字符串字典的key ， key为字符数据的位置


                    if (dic_textData.ContainsKey(curIndex) && curIndex == str_index)
                    {
                        ret.Add(colindex);

                        //dr[colindex] = dic_textData[curIndex];
